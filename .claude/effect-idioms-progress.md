...
- **Iteration 4**: replaced 34 `Effect.succeed(undefined)` → `Effect.void` across `*.spec.ts`. Consolidated `roles/service.ts` `fetchPermissionsFromDb` raw `Effect.tryPromise` onto the shared `makeTryAsync` helper (matches how other services map infrastructure errors).
